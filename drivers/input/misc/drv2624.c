...
#define	INT_MASK				GENMASK(4, 0)
#define	PRG_ERR_MASK				BIT(4)
#define	PROCESS_DONE_MASK			BIT(3)
#define	ULVO_MASK				BIT(2)
#define	OVERTEMPRATURE_MASK			BIT(1)
#define	OVERCURRENT_MASK			BIT(0)

#define	DRV2624_REG_INT_ENABLE			0x02
#define	INT_MASK_ALL				GENMASK(4, 0)
#define	INT_ENABLE_ALL				0
#define	INT_ENABLE_CRITICAL			BIT(3)
...
#define	WORKMODE_MASK				GENMASK(1, 0)
#define	MODE_RTP				0
#define	MODE_WAVEFORM_SEQUENCER			1
#define	MODE_DIAGNOSTIC				2
#define	MODE_CALIBRATION			3
#define	PINFUNC_MASK				GENMASK(3, 2)
#define	PINFUNC_TRIG_PULSE			(0 << 2)
#define	PINFUNC_TRIG_LEVEL			(1 << 2)
#define	PINFUNC_INT				(2 << 2)

#define	DRV2624_REG_CONTROL1			0x08
#define	ACTUATOR_MASK				BIT(7)
#define	ACTUATOR_SHIFT				7
#define	LOOP_MASK				BIT(6)
#define	LOOP_SHIFT				6
#define	AUTOBRK_OK_MASK				BIT(4)
#define	AUTOBRK_OK_ENABLE			BIT(4)

#define	DRV2624_REG_GO				0x0c

#define	DRV2624_REG_CONTROL2			0x0d
#define	LIB_MASK				BIT(7)
#define	LIB_LRA					(0 << 7)
#define	LIB_ERM					(1 << 7)
...
#define	BEMFGAIN_MASK				GENMASK(1, 0)
#define	FB_BRAKE_FACTOR_MASK			GENMASK(7, 4)

#define	DRV2624_REG_DRIVE_TIME			0x27
#define	DRIVE_TIME_MASK				GENMASK(4, 0)
#define	MINFREQ_SEL_MASK			BIT(7)
#define	MINFREQ_SEL_45HZ			(1 << 7)

#define DRV2624_REG_BLK_IDISS_TIME		0x28
#define IDISS_TIME_MASK				GENMASK(3, 0)
#define BLANKING_TIME_MASK			GENMASK(7, 4)
#define BLANKING_TIME_SHIFT			BIT(2)

#define DRV2624_REG_ZC_OD_TIME			0x29
#define ZC_DET_TIME_MASK			GENMASK(1, 0)

#define DRV2624_REG_LRA_OL_CTRL			0x2c
#define LRA_WAVE_SHAPE_MASK			BIT(0)
...
#define	WORK_IDLE				0
#define	WORK_VIBRATOR				BIT(0)
#define	WORK_IRQ				BIT(1)
...
static void dev_init_platform_data(struct drv2624_data *drv2624)
{
	...
	drv2624_set_bits(drv2624, DRV2624_REG_MODE, PINFUNC_MASK, PINFUNC_INT);
...
static int drv2624_parse_dt(struct device *dev, struct drv2624_data *drv2624)
{
	...
	pdata->gpio_nrst = devm_gpiod_get_optional(dev, "reset", 0);
...
static int drv2624_i2c_probe(struct i2c_client *client)
{
	...
	drv2624 = devm_kzalloc(&client->dev, sizeof(*drv2624), GFP_KERNEL);
	if (!drv2624)
		return -ENOMEM;
	...
	if (IS_ERR(drv2624->regmap)) {
		return dev_err_probe(drv2624->dev, PTR_ERR(drv2624->regmap),
	...
	if (err < 0)
		return dev_err_probe(drv2624->dev, err,
				     "i2c error when identifying device\n");
	else
		drv2624->device_id = err;

	if ((drv2624->device_id & 0xf0) != DRV2624_ID) {
		dev_err(drv2624->dev, "device is not a drv2624 (%hhu != %hhu)\n",
			drv2624->device_id & 0xf0, DRV2624_ID);
		return -EINVAL;
	}
